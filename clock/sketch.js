...
function timeToAngle(t, factor) {
  var step = 360 / factor;
  var theta = t % factor * step;
  return radians(theta-90);
}


function drawHand(time, timeFactor, lineWeight, lineFactor, lineColor) {
  var theta = timeToAngle(time, timeFactor);
  var handx = faceSize * lineFactor;
  var handy = faceSize * lineFactor;
...
function clockFace() {
  ...
  for (var i=0; i < 360; i+= 6) {
    var cx = width/2;
    var cy = height/2;
    var x =  cx + faceSize * 0.5 * cos(radians(i));
    var y =  cy + faceSize * 0.5 * sin(radians(i));
